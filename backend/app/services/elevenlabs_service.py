...
"""
from elevenlabs import ElevenLabs, ConversationalConfig, AgentConfig, TtsConversationalConfigOutput
...
class ElevenLabsService:
    ...
    async def create_interview_agent(
        ...
        try:
            # Include resume content directly in the system prompt
            base_prompt = self._get_hardcoded_system_prompt(role)
            system_prompt = f"""{base_prompt}

CANDIDATE'S RESUME:
{resume_text.strip()}

Based on this resume, ask relevant technical questions that match their experience level and background. Reference specific projects, technologies, or experiences from their resume when appropriate."""

            # Empty knowledge base since we're including resume in prompt
            knowledge_base = []
    ...
    async def _create_agent_with_retry(
        ...
        """Create real ElevenLabs conversational AI agent with retry logic."""
        
        last_exception = None
        
        for attempt in range(max_retries):
            try:
                self.logger.info(f"Creating ElevenLabs agent (attempt {attempt + 1}/{max_retries}) for {role} role")
                
                # Create TTS configuration
                tts_config = TtsConversationalConfigOutput(
                    voice_id=voice_id,
                    model_id="eleven_turbo_v2",  # Use the correct turbo v2 model for English agents
                    stability=0.5,
                ...
                prompt_config = PromptAgentApiModelOutput(
                    ...
                    llm="gpt-4o-mini",  # Use efficient model for interviews
                    temperature=0.7,
                    max_tokens=500
                )
                ...
                # Create conversational configuration
                conversation_config = ConversationalConfig(
                    agent=agent_config,
                    tts=tts_config
                )
                
                # Create agent using ElevenLabs Conversational AI API
                agent_response = self.client.conversational_ai.agents.create(
                    name=f"AIVIA {role} Interviewer",
                    conversation_config=conversation_config
                )
                
                self.logger.info(f"Successfully created ElevenLabs agent {agent_response.agent_id} for {role} role")
                ...
                if attempt < max_retries - 1:
                    ...
                    self.logger.error(f"All {max_retries} attempts to create agent failed")
                    
        # If we get here, all retries failed
        raise Exception(f"Failed to create ElevenLabs agent after {max_retries} attempts. Last error: {str(last_exception)}")
    ...
    async def delete_agent(self, agent_id: str) -> bool:
        ...
        try:
            # Delete agent using ElevenLabs API
