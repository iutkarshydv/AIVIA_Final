...
async def start_interview(
    ...
    try:
        ...
        # Validate file type
        if not resume_file.filename or not resume_file.filename.lower().endswith(('.pdf', '.docx', '.doc')):
            raise HTTPException(
                status_code=400,
        ...
        try:
            file_ext = os.path.splitext(resume_file.filename or '')[1]
        ...
        logger.info(f"Successfully started interview session {session_id} for role {role}")

        return InterviewStartResponse(
            session_id=session_id,
            agent_id=agent_result.agent_id,
            websocket_url=websocket_url,
            message=f"Interview session created successfully for {role} role"
        )
...
async def complete_interview(session_id: str, db=Depends(get_db)):
    ...
    try:
        ...
        cleanup_success = await elevenlabs_service.delete_agent(str(session.elevenlabs_agent_id))
...
@app.get("/api/interview/{session_id}/websocket")
async def get_websocket_url(session_id: str, db=Depends(get_db)):
    """
    Get WebSocket URL for real-time conversation.
    """
    try:
        session = await session_manager.get_session(db, session_id)
        if not session:
        ...
        websocket_url = await elevenlabs_service.get_websocket_url(str(session.elevenlabs_agent_id))
        if not websocket_url:
            raise HTTPException(status_code=500, detail="Failed to generate WebSocket URL")
        ...
        logger.error(f"Error getting WebSocket URL for session {session_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
